...
ROFI_CMD="rofi -theme $XDG_CONFIG_HOME/rofi/tools/calc.rasi"
...
manage_wifi() {
	...
	local chosen_network=$(echo -e "$formatted_list" | $ROFI_CMD -dmenu -i -selected-row 1 -p "Wi-Fi SSID: ")
	local ssid_index=-1
	...
	if [ -z "$chosen_network" ]; then
		...
		action=$(echo -e "$action\n  Forget" | $ROFI_CMD -dmenu -p "Action: ")
		case $action in
		"󰸋  Connect")
			...
			if [[ $(echo "$saved_connections" | grep -Fx "$chosen_id") ]]; then
				...
				local wifi_password=$($ROFI_CMD -dmenu -p "Password: " -password)
...
manage_ethernet() {
	...
	local chosen_device=$(echo -e "$eth_list" | $ROFI_CMD -dmenu -i -p "Select Ethernet device: ")
